      ...
      <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
          ...
          <ol class="breadcrumb">
            ...
            <li>
              <a href="#/serviceSettleList">服务单明细</a>
            </li>
            <li>
              服务单明细详情
            </li>
            ...
            <div class="ibox">
              ...
              <div class="ibox-content">
                <div class="container-fluid">
                  <div class="row">
                    <div class="col-md-4" valign="top">
                      <div class="row">
                        ...
                        <div class="col-lg-12"><label>服务费：</label>
                          <span v-if="serviceOrder.parts_money">{{serviceOrder.parts_money | currency}}元</span>
                          <span v-else>无</span>
                        </div>
                        <div class="col-lg-12"><label>实付款：</label>
                          <span v-if="settlement.amount">{{settlement.amount | currency}}元</span>
                          <span v-else>0</span>
                        </div>
                        ...
                        <div class="col-lg-12"><label>车主姓名：</label>
                          <span v-if="cappServOrderCar.workOrderRealName">{{cappServOrderCar.workOrderRealName}}</span>
                          <span v-else>无</span>
                        </div>
                        <div class="col-lg-12"><label>车架号（VIN码）：</label>
                          <span>{{cappServOrderCar.vinNum}}</span>
                        </div>
                        <div class="col-lg-12"><label>入厂里程数：</label>
                          <span v-if="cappServOrderCar.inFactoryMileage">{{cappServOrderCar.inFactoryMileage}}</span>
                          <span v-else>无</span>
                        </div>
                      </div>
                    </div>
                    <div class="col-md-4" valign="top">
                      <div class="row">
                        <div class="col-lg-12"><label>结算金额：</label>
                          <span>{{Samount | currency}}元</span>
                        </div>
                        <div class="col-lg-12"><label>服务车辆：</label>
                          <span>{{serviceOrder.car_num}}</span>
                        </div>
                        <div class="col-lg-12"><label>总金额：</label>
                          <span>{{serviceOrder.amount | currency}}元</span>
                        </div>
                        <div class="col-lg-12"><label>车主ID：</label>
                          <span>{{serviceOrder.cust_vip_num}}</span>
                        </div>
                        <div class="col-lg-12"><label>送修人：</label>
                          <span v-if="cappServOrderCar.repairName">{{cappServOrderCar.repairName}}</span>
                          <span v-else>无</span>
                        </div>
                        <div class="col-lg-12"><label>车主电话：</label>
                          <span v-if="cappServOrderCar.carOwnerPhone">{{cappServOrderCar.carOwnerPhone}}</span>
                          <span v-else>无</span>
                        </div>
                    ...
                    <div class="col-md-4" valign="top">
                      <div class="row">
                        <div class="col-lg-12"><label>服务单号：</label><span>{{serviceOrder.order_num}}</span></div>
                        <div class="col-lg-12">
                          <label>零件费：</label>
                          <span v-if="serviceOrder.service_money">{{serviceOrder.service_money | currency}}元</span>
                          <span v-else>无</span>
                        </div>
                        <div class="col-lg-12">
                          <label>优贝抵扣：</label><span v-if="settlement.quantity">{{settlement.quantity}}</span>
                          <span v-else>0</span>
                        </div>
                        <div class="col-lg-12">
                          <label>注册手机号码：</label>
              ...
              <div class="ibox-content">
                <div class="form-group">
                  <a href="#/serviceSettleList"><button class="btn btn-white">返回</button></a>
                </div>
              </div>
            </div>
          </div>
  ...
  mounted:function () {
    ...
    $.ajax({
      ...
      success:function (data) {
        ...
        self.settlement = self.list.settlement;
        self.Samount = self.list.proportion*self.list.serviceOrder.amount/100
        self.settlement.amount = self.serviceOrder.amount- self.settlement.quantity;
        thisDate(self.list.workPartsListJson);
      },
      error:function () {
